...
export async function makeMap(kaboomContext: KaboomCtx, name: string) {
    ...
    for (const layer of mapData.layers) {
        if (layer.name === "colliders") {
            for (const collider of layer.objects) {
                map.add([
                    kaboomContext.area({
                        shape: new kaboomContext.Rect(kaboomContext.vec2(0), collider.width, collider.height),
                        collisionIgnore: ["platfrom", "exit"],
                    }),
